...
"""Reading file id.txt"""
with open('id.txt', encoding='UTF-8') as f:  # Так удобнее и по всем инструкциям, что я видел лучше,
                                            # так как файл закрывается сам как только заканчивается работа с ним
...
def write_json(data):
    ...
    with open(f'out/' + 'photos.json', 'w') as file:
...
def download_photo(url):
    for owner_id in array_id:
        ...
        with open(f'out/id{owner_id}/' + filename, 'wb') as file:
...
def main():
    ...
    for owner_id in array_id:
        ...
        photos = json.load(open(f'out/' + 'photos.json'))['response']['items']

        for photo in photos:
            sizes = photo['sizes']

            max_size_url = max(sizes, key=get_largest)['url']
            if owner_id in array_id:
